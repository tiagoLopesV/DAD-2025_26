...
const selectedType = ref('standalone')
...
const createNewGame = async () => {
    if (hasActiveTable.value || isCreating.value) return
    ...
    if (userCoins.value < currentCost.value) {
        errorMessage.value = `Saldo insuficiente. Precisas de ${currentCost.value} moedas.`
...
const joinGame = async (game) => {
    if (hasActiveTable.value) return // Prevenção extra

    errorMessage.value = ""
    await coinStore.fetchBalance()

    if (userCoins.value >= game.stake) {
...
<template>
    <div class="container mx-auto p-4 max-w-4xl min-h-screen">
        <div class="space-y-6">
            ...
            <div
                class="flex justify-between items-center bg-slate-900 p-6 rounded-xl border border-slate-800 shadow-xl">
                <div>
                    <h1 class="text-3xl font-black text-white italic tracking-tighter uppercase">Bisca Arena</h1>
                    <p class="text-slate-400 text-xs font-bold uppercase tracking-widest">Lobby Online</p>
                </div>
                <div class="flex items-center gap-4 bg-slate-950 p-3 rounded-lg border border-slate-800">
                    <span class="text-slate-400 text-sm font-bold uppercase tracking-tighter">Saldo:</span>
                    <Badge
            ...
            <Card v-if="hasActiveTable"
                class="border-2 border-blue-500 bg-slate-900 text-white shadow-2xl animate-in zoom-in duration-300">
                <CardHeader class="border-b border-slate-800 pb-4">
                    <CardTitle class="text-lg font-black uppercase flex items-center gap-3">
                        <span class="relative flex h-3 w-3">
                            <span
                                class="animate-ping absolute inline-flex h-full w-full rounded-full bg-blue-400 opacity-75"></span>
                            <span class="relative inline-flex rounded-full h-3 w-3 bg-blue-500"></span>
                        </span>
                        Your Table is Active
                    </CardTitle>
                </CardHeader>
                <CardContent class="pt-6">
                    <div v-for="game in myWaitingGames" :key="game.id"
                        ...
                        <div class="text-center md:text-left">
                            <Badge class="bg-blue-600 mb-2 uppercase text-[10px]">{{ game.type }}</Badge>
                            <h2 class="text-3xl font-black italic uppercase">{{ game.variant }} Cards</h2>
                            <p class="text-yellow-500 font-bold">Stake: {{ game.stake }} 🪙</p>
                        </div>

                        <div class="flex flex-col items-center gap-3 w-full md:w-auto">
                            <div v-if="game.player2" class="text-center space-y-3">
                                <div class="text-green-400 font-black animate-pulse uppercase tracking-widest text-sm">
                                    Opponent Ready!</div>
                                <Button @click="startGame(game)"
                                    class="w-full bg-green-600 hover:bg-green-500 h-12 px-10 text-lg font-black uppercase">Start</Button>
                            </div>
                            <div v-else
                                class="text-slate-500 font-bold animate-pulse text-xs uppercase tracking-widest">Waiting for opponent...</div>

                            <Button @click="cancelGame(game)" variant="destructive" size="sm"
                                class="w-full font-bold uppercase text-[10px]">Cancel Table</Button>
                        </div>
                    </div>
                </CardContent>
            </Card>
                    ...
                    <CardContent class="space-y-6">
                        <div class="grid md:grid-cols-2 gap-8">
                            <div class="space-y-4">
                                <div>
                                    <label
                                        class="text-[10px] font-black mb-2 block text-slate-500 uppercase tracking-widest">Mode</label>
                                ...
                                <div>
                                    <label
                                        class="text-[10px] font-black mb-2 block text-slate-500 uppercase tracking-widest">Cards</label>
                                    <div class="flex gap-2">
                            ...
                            <div class="bg-slate-950 p-4 rounded-xl border border-slate-800">
                                <label
                                    class="text-[10px] font-black mb-3 block text-slate-400 uppercase tracking-widest">Player
                                    Stake</label>
                                ...
                                <div v-else class="space-y-4">
                                    <div class="flex items-center justify-between">
                                        <label
                                            class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Adjust
                                            Stake</label>
                                        <Badge class="bg-blue-500/20 text-blue-400 border-blue-500/50">Min 3 - Max 100
                                        </Badge>
                                    </div>
                                    <div class="flex items-center gap-4">
                                        <Button variant="outline" size="icon"
                                            @click="matchStake = Math.max(3, matchStake - 1)"
                                            class="h-8 w-8 rounded-full border-slate-700 bg-slate-800 text-white"> -
                                        </Button>
                                        ...
                                        <Button variant="outline" size="icon"
                                            @click="matchStake = Math.min(100, matchStake + 1)"
                                            class="h-8 w-8 rounded-full border-slate-700 bg-slate-800 text-white"> +
                                        </Button>
                                    </div>
                                    <div
                                        class="bg-slate-900/50 p-3 rounded-lg border border-slate-800 flex justify-between items-center">
                                        <div class="flex flex-col">
                                            <span class="text-[10px] text-slate-500 font-bold uppercase">Total
                                                Stake</span>
                                            <span class="text-2xl font-black text-blue-500">{{ matchStake }} 🪙</span>
                                        </div>
                                        <div class="text-right">
                                            <span class="text-[10px] text-slate-500 font-bold uppercase">Estimated
                                                Prize</span>
                                            <span class="text-xl font-black text-green-500 block">{{ (matchStake * 2) -
                                                1 }} 🪙</span>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div v-else class="grid gap-3">
                            <div v-for="game in availableGames" :key="game.id"
                                ...
                                <div class="text-right flex flex-col gap-2">
                                    <span class="text-xs font-bold text-yellow-500 uppercase">Stake: {{ game.stake }}
                                        🪙</span>
